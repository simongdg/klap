...
inline __device__ unsigned Graph::getInDegree(unsigned dst) {
	if (dst < nnodes) {
...
inline __device__ unsigned Graph::getDestination(unsigned src, unsigned nthedge) {
	unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
...
inline __device__ foru Graph::getWeight(unsigned src, unsigned nthedge) {
	unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
	if (src < nnodes && nthedge < getOutDegree(src)) {
		unsigned edge = getFirstEdge(src) + nthedge;
		if (edge && edge < nedges + 1) {
...
inline __device__ unsigned Graph::getFirstEdge(unsigned src) {
	unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
	if (src < nnodes) {
...
inline __device__ unsigned Graph::getMinEdge(unsigned src) {
	unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
	if (src < nnodes) {
		unsigned srcnout = getOutDegree(src);
		if (srcnout > 0) {
...
inline __device__ void Graph::print1x1() {
...
inline unsigned Graph::allocOnHost() {
...
inline unsigned Graph::allocOnDevice() {
...
inline unsigned Graph::deallocOnHost() {
	free(noutgoing);
	free(nincoming);
	free(srcsrc);
	free(psrc);
...
inline unsigned Graph::deallocOnDevice() {
	cudaFree(noutgoing);
	cudaFree(nincoming);
	cudaFree(srcsrc);
	cudaFree(psrc);
...
inline unsigned Graph::optimizeone() {
...
inline unsigned Graph::optimizetwo() {
	// load balance.
...
inline unsigned Graph::optimize() {
	optimizeone();
	optimizetwo();
	return 0;
}
inline void Graph::progressPrint(unsigned maxii, unsigned ii) {
...
inline unsigned Graph::readFromEdges(char file[]) {
	std::ifstream cfile;
	cfile.open(file);
...
inline unsigned Graph::readFromGR(char file[]) {
	std::ifstream cfile;
	cfile.open(file);
...
inline unsigned Graph::read(char file[]) {
...
inline long unsigned Graph::cudaCopy(struct Graph &copygraph) {
...
inline __device__ void Graph::computeStats() {
	computeInOut();
	computeDiameter();
}
inline __device__ bool Graph::computeLevels() {
	unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
	bool changed = false;
...
inline __device__ unsigned Graph::findMaxLevel() {
	unsigned maxlevel = 0;
	for (unsigned ii = 0; ii < nnodes; ++ii) {
...
inline __device__  void Graph::computeDiameter() {
	diameter = findMaxLevel();
}
inline __device__ void Graph::computeInOut() {
	for (unsigned ii = 0; ii < nnodes; ++ii) {
		// process outdegree.
...
inline __device__ void Graph::printStats1x1() {	// 1x1.
	char prefix[] = "\t";
 ...
 * Global functions.
 */
static __global__ void dprintstats(Graph graph) {
	graph.printStats1x1();
}
static __global__ void dcomputelevels(Graph graph, bool *changed) {
	if (graph.computeLevels()) {
		*changed = true;
	}
}
static __global__ void dinitlevels(Graph graph) {
	graph.initLevels();
}
static __global__ void dprint1x1(Graph graph) {
	graph.print1x1();
}
...
inline unsigned Graph::printStats() {
	allocLevels();
